...
1. 用 rizin（Ghidra/IDA 亦可）打开 .so 文件
2. 在 Exports 窗口/符号表找到目标函数
3. 函数起始地址：rizin 显示的函数头地址
...
将 patched .so 文件在 rizin 中打开（Ghidra/IDA 亦可）：
1. 用 rizin 的 `s <addr>` 跳转到目标函数地址
2. 用 `af`/`afb` 重新识别函数边界（如需要）
3. 用 rizin 反汇编（伪代码可用 Ghidra headless 回退）查看结果
