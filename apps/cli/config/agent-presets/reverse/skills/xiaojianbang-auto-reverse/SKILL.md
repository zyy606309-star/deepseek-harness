...
15. **定制系统能力硬门禁（按设备判定）**：定制系统设备为 `19051FDF60018V`，普通 root 设备为 `11FAFS00000VYM`。连接/指定定制系统设备时可直接用整体/抽取式脱壳、任意 so 注入、native 注册监听、内置 Apatch root；连接普通 root 设备 `11FAFS00000VYM` 或未确认设备时，不启用这些能力、走通用流程。任务中用户会说明用哪台设备；实验记录须写明当前设备序列号。`syscall-filter`、`stealth-hook` 不属于定制系统能力，只按 APatch/KernelPatch/KPM/root 等通用前置条件判断。详见 `references/custom-system.md`、`references/environment.md`。
