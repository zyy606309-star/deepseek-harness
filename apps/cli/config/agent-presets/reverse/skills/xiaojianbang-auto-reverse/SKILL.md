...
20. **jadx / 反汇编工具路径未命中必须全盘搜索**：PATH、项目目录、已有记录和常见安装路径都找不到 `jadx` 或 `rizin`/`rz-bin` 时，必须做宿主机全盘搜索并记录命令、范围、候选和结果；全盘仍找不到才询问用户路径或请求确认回退。详见 `references/tooling-and-paths.md`。
