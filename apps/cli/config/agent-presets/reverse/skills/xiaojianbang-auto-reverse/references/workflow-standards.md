...
- **先 hook dlopen 定位"处决逻辑"所在 so（本文提炼）**：遇到反 Frida/反调试，不要上来就全局分析。先 hook 加载相关函数（`dlopen`/`__loader_dlopen`/`android_dlopen_ext`，符号在 `libc.so`/`libdl.so`/`linker64`/`linker` 里查），打出"哪个 so 被加载 + 加载时发生了什么"，从而定位"处决逻辑（检测到就 kill/上报）"到底在哪个 so。这样缩小分析范围，再针对该 so 做 dump/trace。
- **sleep 应对检测"处决时间滞后"（本文提炼）**：不少检测逻辑是"检测到异常后**不立刻处置**，而是**等一下再 kill**"（处决滞后）。因此在关键触发点（如每次 dlopen enter 前）`Thread.sleep(2s)`，可以给自己争取时间，避免还没 dump/trace 就被 kill。若 sleep 导致崩溃，用**二分查找**缩小到合适时机。
- **trace 定位检测点（本文提炼）**：顺着处决点向上 trace，找上游检测逻辑。注意 Stalker 日志要打在**运行时（执行到函数时）**，不能写死在 `transform()` 里（那是 block 编译阶段，trace 会失真/无效）。trace 可让 AI 生成；若某函数未加载会抛异常，需处理未加载情形；可结合 sleep + 二分查找收敛到检测函数。难定位时用 KPM 模块或 eBPF 程序 trace，对比 Frida trace 函数链**在哪出现路径分叉**，再往上追。
...
**共用函数必须"精确 caller 条件"再改（本文提炼）**：被多处复用的函数，**不能无脑改返回**。它可能除了干检测，还被正常流程调用（如某函数既被检测/VM 调度用，又被 `JNI_OnLoad` 用去返回 `JNI_VERSION`）。无脑改 0 会弄坏正常功能（如 JNI 初始化失败、App 起不来）。正确做法：**只在 caller 落入特定检测区间才改**（如 caller 落在 `0x5ccf8..0x5ce00` 才把返回改 0），否则原样放行——在 `Interceptor.attach` 里判断 `returnAddress`/caller 归属，再决定是否 patch。

**持续清检测状态位（本文提炼）**：有些壳用**全局状态位/状态变量**记录"检测命中"，并有 VM 调度器把检测状态写成固定值（如 `+0x338` = 检测状态 17）。单纯 hook 检测函数可能不够，应同时**持续清这些状态位**（`keepClean()` 循环清 `state+0x338/+0x1d1/+0x33e`），让状态永远是"干净"。这比逐个堵返回更彻底——即使有漏网的检测把状态置了，也被持续清掉。

**多窗口安装 hook/patch（本文提炼）**：不要只在单一时机装 hook。在 **`malloc`、`mmap`（如 raw-clone 大块分配）、`pthread_create`（创建线程、且 start routine 是目标）** 等多个"安全窗口"批量/补装，抓不同时机见缝插针，确保无论检测从哪个窗口触发，关键 hook/patch 都已就位。
...
**决策纪律：patch 后仍反复被 SIGKILL，先抬头查进程树再继续逆编码（本文提炼）**：主进程内 patch 完仍反复被杀（尤其 signal 9 时间稳定），**不要继续低头逆加密**——这是岔路口强信号，说明**检测可能不在当前进程里，而在独立检测子进程**。此时必须先停手，查进程归属：`ps` 看同名多进程、hook `clone`/`fork` 确认有无 fork 出独立检测子进程、确认子进程**重新解密/加载了哪份内层 ELF**。**"patch 主进程"≠"过了检测"**——子进程常重新解密内层 ELF、基址不同，父进程内 patch 覆盖不到（见 `shell-signatures.md` 特征 20）。确认检测在哪个进程后，再决定 patch 落点（含击杀/冻结检测载体进程本身）；**击杀检测载体进程 = 让检测函数不再执行，属进程级绕过，不违背"隐藏 root 环境"的边界**。这条把"脑里有知识点"变成"会按信号触发查进程"的动作纪律，避免在错误进程里死磕同一 patch。
