...
- 新加载 so 监控、加密 so 内存 dump 修复、rizin 反汇编（伪代码 Ghidra headless 回退）导出分析、OLLVM/非标准控制流还原。
...
安装脚本默认保留已有工具目录和 runner，且不写入 IDA 插件目录，也不把 `INP.py` 复制到项目 `scripts/`。本模式默认用 `rizin_export.py` 导出（伪代码加 `--ghidra-support`），它在 `--with-runner` 时一并复制；只有用户明确要用 IDA 时，才用 `--with-inp`/`--install-ida-plugin` 复制 `INP.py`。
...
scripts/rizin_export/DecompileHeadless.java（Ghidra 伪代码回退脚本）
scripts/INP.py（仅显式 --with-inp / --install-project-inp 时）
<IDA目录>/plugins/INP.py（仅显式安装 IDA 插件时）
```
...
| `rizin_export.py` | rizin 导出 AI 输入（反汇编、字符串、导入导出、符号、段、xref、函数列表）；加 `--ghidra-support` 用 Ghidra headless 出伪代码 | rizin/rz-bin；伪代码需 Ghidra headless + JDK 21+ |
...
- `references/tool-installation.md`：工具复制、自检、审计、`rizin_export.py`/`INP.py` 复制。
- `references/rizin-tools.md`：本模式默认工具链（rizin 反汇编 + Ghidra headless 伪代码回退）。
- `references/environment.md`：本机/当前测试机工具链环境事实（模板）。
- `references/syscall-frida-tools.md`：syscall-filter、Frida 联合采集、关键证据提取。
- `references/dump-ida-ollvm-tools.md`：MemDumper、rizin 导出、函数范围修正、OLLVM。
