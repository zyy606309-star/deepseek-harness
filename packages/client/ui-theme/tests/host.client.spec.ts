...
describe('ui-theme host', () => {
  it('registers, validates, and disposes the durable theme namespace with its fiber', async () => {
    ...
    expect(ctx.settings.get(ns)).toEqual({ preference: DEFAULT_PREFERENCE, backgroundImage: '', backgroundOpacity: 0.58, fontScale: 1 })
    await ctx.settings.update(ns, { preference: 'dark' })
    expect(ctx.settings.get(ns)).toEqual({ preference: 'dark', backgroundImage: '', backgroundOpacity: 0.58, fontScale: 1 })
  ...
  it('renders the current durable preference and disposes the index transform', async () => {
    ...
    expect(transform?.('<body></body>')).toContain('--dsw-bg-image')
    expect(transform?.('<body></body>')).toContain('--dsw-surface-opacity')
    ...
    expect(transform?.('<body></body>')).toContain('https://example.com/bg.png')
    expect(transform?.('<body></body>')).toContain('"50%"')
    await fiber.dispose()
