...
describe('ThemeRuntime', () => {
  ...
  it('adopts a published background section', () => {
    ...
    host.publish({ status: 'ready', value: { preference: 'system', backgroundImage: 'https://example.com/bg.png', backgroundOpacity: 0.3, fontScale: 1 }, revision: 1, writable: true })
  ...
  it('adopts a published Host section without writing it back', () => {
    const { theme, events, host } = make()
    host.publish({ status: 'ready', value: { preference: 'dark', backgroundImage: '', backgroundOpacity: 1, fontScale: 1 }, revision: 1, writable: true })
    expect(theme.getTheme().preference).toBe('dark')
    expect(events).toHaveLength(1)
    expect(host.set).not.toHaveBeenCalled()
    host.publish({ value: { preference: 'dark', backgroundImage: '', backgroundOpacity: 1, fontScale: 1 }, revision: 2 })
    expect(events).toHaveLength(1)
  })
