...
describe('createAppearanceRowStore', () => {
  it('init shape: system preference, no background, revision at -1', () => {
    ...
    expect(store.getSnapshot()).toEqual({
      preference: 'system', backgroundImage: '', fontScale: 0, effectiveFontScale: 1, revision: -1,
    })
  })

  it('sync mirrors the preference and background and advances the revision', () => {
    const store = createAppearanceRowStore().create()
    store.actions.sync('dark', 'https://example.com/bg.png', 1, 1, 0)
    expect(store.getSnapshot()).toEqual({
      preference: 'dark', backgroundImage: 'https://example.com/bg.png', fontScale: 1, effectiveFontScale: 1, revision: 0,
    })
    store.actions.sync('light', '', 0, 1.1, 2)
  ...
  it('revision guard drops stale and duplicate writes', () => {
    const store = createAppearanceRowStore().create()
    store.actions.sync('dark', 'a', 1, 1, 3)
    store.actions.sync('system', 'b', 0, 1, 2)
    store.actions.sync('system', 'b', 0, 1, 3)
