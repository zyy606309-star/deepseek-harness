...
describe('AppFrame', () => {
  ...
  it('drag base is the rendered (concession-clamped) width, not the preference', () => {
    ...
    const handles = frame.querySelectorAll('[class*="handle"]')
    const handleX = frameWidth - 330
    drag(handles[1]!, handleX, handleX + 10) // dragging the details edge right shrinks it by 10
...
describe('AppFrame — guard branches', () => {
  ...
  it('two moves inside one frame coalesce through the pending rAF', () => {
    ...
    act(() => {
      ...
      handle.dispatchEvent(new PointerEvent('pointermove', { pointerId: 1, clientX: SIDEBAR_DEFAULT + 20, bubbles: true }))
      handle.dispatchEvent(new PointerEvent('pointermove', { pointerId: 1, clientX: SIDEBAR_DEFAULT + 20, bubbles: true }))
      vi.advanceTimersByTime(20)
    })
    act(() => { handle.dispatchEvent(new PointerEvent('pointerup', { pointerId: 1, clientX: SIDEBAR_DEFAULT + 20, bubbles: true })) })
    expect(instance.getSnapshot().sidebar).toBe(SIDEBAR_DEFAULT + 20)
  })
  ...
  it('pointerup with a pending rAF cancels it and commits the final position', () => {
    ...
    act(() => { handle.dispatchEvent(new PointerEvent('pointerdown', { pointerId: 1, clientX: SIDEBAR_DEFAULT, bubbles: true })) })
    act(() => {
      handle.dispatchEvent(new PointerEvent('pointermove', { pointerId: 1, clientX: SIDEBAR_DEFAULT + 40, bubbles: true }))
      // No timer advance: the rAF is still pending when pointerup arrives.
      handle.dispatchEvent(new PointerEvent('pointerup', { pointerId: 1, clientX: SIDEBAR_DEFAULT + 40, bubbles: true }))
    })
    expect(instance.getSnapshot().sidebar).toBe(SIDEBAR_DEFAULT + 40)
  })
