...
import {
  ...
  relativeTime, StateDot, writeClipboard,
...
export function SessionNodeItem({
  ...
  // (~/.dsh/sessions/<workspace>/<session id>/), so it survives renames and is
  // what another session quotes to hand work over.
      ...
      {!row.blank && (
        <span className={css.rowActions}>
          <Menu
            ...
            onSelect={(id) => {
              // The copy row keeps the menu open so its "Copied" feedback is
              // visible; every other row closes first.
              if (id === 'copyId') {
                copySessionId()
                return
              }
