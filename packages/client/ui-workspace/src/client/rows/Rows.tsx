 ...
 */
export function SessionNodeItem({ node, currentId, now, onOpen, onRename, onFork, onArchive, onRelease, drag, flat = false, t }: {
  node: SessionNode
  currentId: string | undefined
  now: number
  ...
  /** Release this session's resident host agent from memory (row menu action). */
  onRelease: (id: SessionNode['id']) => void
  ...
  const sessionMenuItems = [
    ...
    // Release only while the session is idle (no running turn or descendant):
    // a busy session must never be offered for teardown.
    ...(node.running || node.runningSubagentCount > 0
      ? []
      : [{ id: 'release', label: t('menu.releaseSession'), icon: <IconRefreshOutline16 /> }]),
  ]
        ...
        >
          <Menu
            ...
            onSelect={(id) => {
              ...
              if (id === 'release') onRelease(node.id)
            }}
            portal
            closeOnPointerLeave
            anchor={(
