...
- **Routed policy** — proactive pressure resolves capacity from the adapter that owns the incoming provider/model route (the pre-step selection when installed, otherwise the latest durable route), then scales the default policy plus an optional exact-target override into concrete token budgets. Model discovery remains advisory and is not consulted.
...
|---|---|---|
| `thresholdRatio` | no (default `0.7`) | Compact at `floor(routedContextWindow × ratio)`. |
...
| `maxOverflowRetries` | no (default `3`) | Maximum retries after canonical context-window overflow; `0` disables recovery only. |
