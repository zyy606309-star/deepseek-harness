...
export function spawnSubprocess(spec: SubprocessSpawnSpec, internals: SpawnInternals = {}): LocalSubprocessHandle {
  ...
  const child = spawn(program, args, {
    ...
    // Hide the child's console window: a console-less host (e.g. `dsh web`
    // under a desktop wrapper) would otherwise let Windows allocate each child
    // a visible window that flashes and closes.
    windowsHide: true,
  })
