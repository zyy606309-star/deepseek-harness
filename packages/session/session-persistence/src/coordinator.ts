...
import { SessionFormatUnsupportedError, sessionFormatVersionRefusal } from './format-refusal.ts'

export { SessionFormatUnsupportedError, sessionFormatVersionRefusal } from './format-refusal.ts'
 ...
 * remains readable at {@link SessionFormatUnsupportedError#location} when the
 * backend keeps one artifact per session.
 */
...
export class PersistenceCoordinator<TornMarker = unknown> {
  ...
  private assertEventsSupported(meta: SessionHeader, events: readonly SessionEvent[]): void {
    assertKnownEventTypes(events, meta.id, this.backend.locate?.(meta))
  }
