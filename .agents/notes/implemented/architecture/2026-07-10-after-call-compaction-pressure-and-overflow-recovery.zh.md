...
`maxOverflowRetries` 可选且默认为 `3`；`0` 只禁用溢出恢复，不会禁用压力检查。默认允许三次恢复尝试，因为当溢出在长会话后期才被发现时，一次收缩并不总能把会话带回窗口以内；每次尝试仍要求可见的表层 generation 变化，因此该上限只约束可尝试的独立缩减次数。`auto: false` 不注册任何自动监听器。非规范化错误、尝试耗尽、已经中止的 signal、缺失路由模型、没有安全范围、generation 未变化，以及在任何替换之前恢复抛错，都会委托给下一个监听器。若没有后续恢复，循环报告原始提供方错误对象与代码。generation 增加后的恢复抛错会基于持久进展授权重试；即使恢复工作并发完成，取消或 dispose（资源释放）仍具有最终优先级。
