...
对于主动压力检查，compaction-basic 读取即将到来的路由——安装了预步骤模型选择时为该选择，否则为最新持久请求路由——解析其适配器容量与精确目标策略，再把比例缩放为 `ResolvedCompactSpec`。每次检查都会重新解析，因此同一会话切换提供方或模型后，容量与策略会立即变化，甚至在下次请求持久化该切换之前就已生效。若绝对保留预算不小于缩放后的阈值，系统会在目标容量首次允许比较两者时失败。
