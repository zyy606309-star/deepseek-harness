...
For proactive pressure, compaction-basic reads the incoming route — the pre-step model selection when one is installed, otherwise the latest durable request route — resolves its adapter capacity and exact-target policy, and scales ratios into a `ResolvedCompactSpec`. It performs this resolution on every check, so a provider or model switch in one session changes capacity and policy immediately, even before the switch is persisted by the next request. An absolute retained budget that is not below the scaled threshold fails when the target capacity first makes that comparison possible.
