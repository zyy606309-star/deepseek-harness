...
- The escalation fields exist exactly when the mounted executor confines; a request for the SAME mode the call already runs under is an idempotent grant (no approval, the call proceeds as-is), while a request that is neither the effective mode nor strictly wider (a downgrade) fails closed with its own text and prompts no one; a deployment with no ApprovalService fails escalating calls closed and leaves plain calls untouched.
...
- **The advertised target set is static while the effective mode is per-session** (schemas are registry-global) — a session already at the widest mode is still offered the fields. Harmless by construction: the strict-wider check at execution, not the enum, is the safety boundary — a same-mode ask is an idempotent grant, while a non-widening downgrade fails with its own text and never prompts anyone.
