...
本节能力仅在目标 Android 设备是**小肩膀定制系统**时可用。当前部署的定制系统设备序列号为 **`19051FDF60018V`**；连接或指定该设备时可直接使用本节能力，无需每次询问。任务中用户会说明使用哪台设备（定制系统 `19051FDF60018V` / 普通 root `11FAFS00000VYM`）。

- 连接的是定制系统设备（`19051FDF60018V`）：可使用本节的开箱即用功能、手动启用功能和 `dexfix_runner.py`。
- 连接的是普通 root 设备（`11FAFS00000VYM`）或未确认：不要假设设备具备定制系统能力，按通用流程（Frida、MemDumper、syscall-filter、stealth-hook 等）推进，并在实验记录写明当前设备。
...
- 当前连接设备未确认（既不是已知定制系统 `19051FDF60018V`，也不确定是普通 root 设备）：禁止使用整体/抽取式脱壳、任意 so 注入、native 注册监听、定制系统内置 Apatch root、`dexfix_runner.py` 等依赖定制系统能力；先按通用流程 + 实验记录写明设备。
- 实验记录没有写入当前设备序列号：视为门禁未通过，先补记录再继续。
