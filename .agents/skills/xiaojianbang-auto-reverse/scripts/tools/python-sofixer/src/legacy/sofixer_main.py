#!/usr/bin/env python3
"""
SoFixer Main Program
====================

Entry point for the SoFixer tool - repairs dumped SO files from memory.
This is the main program that coordinates all the ELF processing modules.

Originally a monolithic file, now split into focused modules:
...
Usage:
    python sofixer_main.py -s dumped.so -o fixed.so -m 0x7DB078B000 -d
"""
...
Examples:
  ...
  python sofixer_main.py -s dumped.so -o fixed.so -m 0x7DB078B000

  # With debug output and base SO file
  python sofixer_main.py -s dumped.so -o fixed.so -m 0x7DB078B000 -d -b original.so
        """
    )
